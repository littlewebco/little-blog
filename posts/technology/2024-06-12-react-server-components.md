...
- Components render on server Ã¢ÂÂ HTML sent to client Ã¢ÂÂ Hydrated on client
- JavaScript bundle still needs to be downloaded

Server Components:
- Components run on server Ã¢ÂÂ Serialized and sent to client Ã¢ÂÂ No hydration needed
...
Ã¢ÂÂ **Data fetching** - Components that fetch and display data
Ã¢ÂÂ **Static content** - Content that doesn't change frequently
Ã¢ÂÂ **Heavy computations** - Work that's better done on server
Ã¢ÂÂ **Large dependencies** - Libraries that would bloat the bundle

### When to Avoid

Ã¢ÂÂ **Interactive components** - Need event handlers
Ã¢ÂÂ **Browser APIs** - Window, document, localStorage
Ã¢ÂÂ **Real-time updates** - WebSockets, subscriptions
Ã¢ÂÂ **State management** - Components that need useState, useEffect
